export default function PromosBySection() {
  ...
  return (
    <section className="flex flex-wrap w-full justify-center gap-12">
      {cards.map((e, i) => (
        <div
          key={i}
          className="flex flex-wrap w-[40vw] gap-8  justify-start text-white"
        >
          <div className=" max-w-[180px] min-w-[180px]  ">
            <span>{e.category}</span>
            <h3 className="text-cyan-400 text-2xl">{e.title}</h3>
            <button className="w-fit h-8 mt-4">{e.button}</button>
          ...
          <img
            className="[aspect-ratio:1/1] w-60 h-auto outline outline-1 outline-offset-8 outline-cyan-900"
            src={e.image}
            alt={e.title + " image"}
          />
        </div>
